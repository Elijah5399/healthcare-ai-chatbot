...
userSchema.statics.signup = async function(name, email, password) { // creating an additional static method for usersModel that can be used
    ...
    if (!validator.isStrongPassword(password, { minNumbers: 0, minSymbols: 0 })) {
        throw Error("Password must be at least 8 characters long with at least 1 upper and lower case character and 1 number and 1 special character. Please ensure that there is no lexicographical sequence of characters as well")
    }
...
userSchema.statics.login = async function(email, password) {
    ...
    const user = await this.findOne({ email }) // a second layer of check to ensure that emails are unique
